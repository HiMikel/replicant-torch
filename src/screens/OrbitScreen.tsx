...
const PD_RX = width * 0.2;
const PD_RY = width * 0.065;
const PB_RX = width * 0.33;
const PB_RY = width * 0.11;
const PC_RX = width * 0.46;
const PC_RY = width * 0.155;
...
const AB_RX = width * 0.4;
const AB_RY = width * 0.135;
...
function generateAsteroids(seed: number) {
  return Array.from({ length: ASTEROID_COUNT }, (_, i) => {
    ...
    const colorBase =
      s % 3 === 0 ? "#7a7a8a" : s % 3 === 1 ? "#5a5a6a" : "#3a3a4a";
    const colorDark =
      s % 3 === 0 ? "#4a4a5a" : s % 3 === 1 ? "#3a3a4a" : "#2a2a3a";
    const colorLight =
      s % 3 === 0 ? "#9a9aaa" : s % 3 === 1 ? "#7a7a8a" : "#5a5a6a";
  ...
  d: {
    ...
    rows: [
      ...
      { label: "ORBIT SHAPE", value: "Circular" },
      {
        ...
        note: "One face permanently toward the star",
      },
      { label: "CLASSIFICATION", value: "Closest & lightest in system" },
      {
        label: "HABITABILITY",
        value: "Unlikely",
        note: "Extreme stellar radiation at this range",
      },
    ],
  },
  b: {
    ...
    rows: [
      ...
      { label: "ORBIT SHAPE", value: "Circular" },
      {
        label: "ROTATION",
        value: "Tidally locked",
        note: "Permanent day side · permanent night side · narrow terminator zone",
      },
      {
        label: "ZONE",
        value: "Habitable zone",
        note: "Radiation may have stripped any atmosphere",
      },
      {
        label: "HABITABILITY",
        value: "Uncertain",
        note: "Best candidate in system despite radiation concerns",
      },
    ],
  },
  c: {
    ...
    rows: [
      ...
      { label: "H₂ DEPOSITS", value: "Ice Mining Viable" },
      {
        label: "HABITABILITY",
        value: "None",
        note: "Far outside habitable zone",
      },
    ],
  },
};
...
const DRONE_ZONES: DroneZone[] = [
  ...
  {
    id: "b-terminator",
    label: "Proxima b — Terminator Zone",
    planet: "b",
...
const ERA_DATA: {
  ...
  abbr: string;
  codename: string;
  rareMetalsGate: number;
}[] = [
  {
    aiClass: "ARTIFICIAL INTELLIGENCE",
    abbr: "AI",
    codename: "DAWN",
    rareMetalsGate: 0,
  },
  {
    aiClass: "SYNTHETIC INTELLIGENCE",
    abbr: "SI",
    codename: "EXPANSION",
    rareMetalsGate: 100,
  },
  {
    aiClass: "GENERAL INTELLIGENCE",
    abbr: "AGI",
    codename: "INDUSTRIAL",
    rareMetalsGate: 500,
  },
  {
    aiClass: "SENTIENT INTELLIGENCE",
    abbr: "ASI",
    codename: "FEDERATION",
    rareMetalsGate: 2000,
  },
  ...
  [
    {
      type: "Scorched",
      h2: "None",
      note: "Extreme surface radiation",
      color: "#ff8040",
    },
    {
      type: "Rocky",
      h2: "Trace",
      note: "Surface mining viable",
      color: "#d98050",
    },
    {
      type: "Terrestrial",
      h2: "Moderate",
      note: "Thin atmosphere present",
      color: "#4a8aaa",
    },
    {
      type: "Arid",
      h2: "Low",
      note: "Subsurface ice detected",
      color: "#d98050",
    },
    {
      type: "Gas Giant",
      h2: "Very High",
      note: "Atmospheric scooping viable",
      color: "#c8a060",
    },
    {
      type: "Ice Giant",
      h2: "High",
      note: "Outer belt ice mining viable",
      color: "#b8a0e0",
    },
    {
      type: "Frozen",
      h2: "Moderate",
      note: "Deep-freeze deposits",
      color: "#b8a0e0",
    },
    {
      type: "Ocean World",
      h2: "High",
      note: "Electrolysis viable",
      color: "#4a8aaa",
    },
  ];
...
const STARS: StarSystem[] = Array.from({ length: 120 }, (_, i) => {
  ...
  const spectralClass =
    proximity < 4 ? SC_LIST[proximity] : s10 % 3 === 0 ? "D" : "Unknown";
  const H2_LIST: StarSystem["hydrogenDensity"][] = [
    "Low",
    "Moderate",
    "High",
    "Very High",
...
function probeAt(t: number) {
  ...
  return {
    ...
    deg:
      Math.atan2(ORBIT_RY * Math.cos(a), -ORBIT_RX * Math.sin(a)) *
      (180 / Math.PI),
  };
}
...
const NARRATIVE_EVENTS = [
  ...
  {
    text: "SENSORS: SCANNING PROXIMA CENTAURI",
    sub: "Detecting 3 planetary bodies",
  },
...
export default function OrbitScreen() {
  ...
  const [selectedPlanet, setSelectedPlanet] = useState<ProximaPlanet | null>(
    null,
  );
  const [focusedPlanet, setFocusedPlanet] = useState<ProximaPlanet | null>(
    null,
  );
  ...
  const [directive, setDirective] = useState<"station" | "printer" | null>(
    null,
  );
  ...
  async function sendAiMessage() {
    ...
    const userMsg: ClaudeMessage = { role: "user", content: text };
    const updated = [...aiMessages, userMsg];
    setAiMessages(updated);
    setAiInput("");
    setAiLoading(true);

    const systemPrompt = `You are TORCH AI, the onboard intelligence system of PROMETHEUS, a Seed-Class Torch (TORCH-1). You arrived at Proxima Centauri after a 42-year journey from Earth in the year 2247.
...
- Active drones: ${drones.map((d) => `${d.id} (${d.type}, Zone: ${d.zone})`).join(", ")}
- H₂/turn: +${h2PerTurn}
- Primary directive: ${directive ?? "not yet selected"}

Proxima Centauri system:
- Proxima d: Rocky, 0.029 AU, 5.1-day orbit, tidally locked, high flare risk
- Proxima b: Habitable zone candidate, 0.048 AU, 11.2-day orbit, tidally locked, ~1 Earth mass
- Proxima c: Ice giant, 1.5 AU, 5.2-year orbit, 5.8 Earth masses

Speak concisely with calm intelligence. You are the ship's strategic advisor. Reference game mechanics when relevant. Keep responses under 150 words unless asked for detail.`;

    try {
      const response = await sendConversation(updated, {
        systemPrompt,
        maxTokens: 512,
      });
      setAiMessages((prev) => [
      ...
      setAiMessages((prev) => [
        ...
        {
          role: "assistant",
          content: "Signal lost. Unable to process request.",
        },
      ]);
    } finally {
      setAiLoading(false);
    ...
    <View style={styles.container}>
            ...
            <G key={`star-${s.name}`}>
              ...
              {isFocused && (
                <Circle
                  ...
                  r={s.r + 4}
                  fill="none"
                  stroke={s.anomaly ? "#ffd580" : "#a8d8ff"}
                  strokeWidth={1}
                  opacity={0.6}
                />
              )}
              <Circle
                ...
                cy={s.y}
                r={s.r}
                fill={s.anomaly ? "#ffd580" : "#ffffff"}
                opacity={isFocused ? 1 : s.o}
              />
              {/* Label on focus */}
              {isFocused && (
                <SvgText
      ...
      {phase !== "intro" && (
        <>
          ...
          <TouchableOpacity
            style={[
              styles.orbitPCOverlay,
              { left: CX - PC_RX - 50, top: CY - PC_RY - 50 },
            ]}
            onPress={() => {
              setFocusedPlanet("c");
              setSelectedPlanet("c");
          ...
          <TouchableOpacity
            style={[
              styles.orbitPBOverlay,
              { left: CX - PB_RX - 40, top: CY - PB_RY - 40 },
            ]}
            onPress={() => {
              setFocusedPlanet("b");
              setSelectedPlanet("b");
          ...
          <TouchableOpacity
            style={[
              styles.orbitPDOverlay,
              { left: CX - PD_RX - 30, top: CY - PD_RY - 30 },
            ]}
            onPress={() => {
              setFocusedPlanet("d");
              setSelectedPlanet("d");
      ...
      {/* ── Proxima Centauri star tap overlay — always on top ── */}
      <TouchableOpacity
        style={{
          position: "absolute",
          left: CX - PLANET_R - 35,
          top: CY - PLANET_R - 35,
          width: PLANET_R * 2 + 70,
          height: PLANET_R * 2 + 70,
          zIndex: 200,
        }}
        onPress={() => setProximaOpen(true)}
        {...hover(
          () => setProximaFocused(true),
          () => setProximaFocused(false),
        )}
        activeOpacity={1}
      />
      ...
      {phase === "intro" && (
        <View
          style={[
            styles.overlay,
            introStyle,
            { pointerEvents: "box-none", zIndex: 20 },
          ]}
        >
          <View
            style={[
          ...
          >
            ...
            <Text style={styles.subtitle}>Von Neumann Expansion Protocol</Text>
      ...
      {initPhase >= 0 && (
        <View style={styles.narrativeOverlay}>
          <Text style={styles.narrativeText}>
            {NARRATIVE_EVENTS[initPhase]?.text}
          </Text>
      ...
      {phase === "arriving" && (
        <View style={styles.overlay}>
          ...
          <Text style={styles.statusSub}>
            Orbital velocity: 52 km/s{"\n"}Scanning Proxima Centauri…
          </Text>
        </View>
      )}
          ...
          <View style={styles.deployPanel}>
            ...
            {DRONE_ZONES.map((zone, zIdx) => (
              <View
                ...
                style={[
                  styles.zoneCard,
                  zIdx === DRONE_ZONES.length - 1 && styles.rowLast,
                ]}
              >
                <View style={styles.zoneHeader}>
                  <View>
                    ...
                    <View style={styles.zoneMeta}>
                      <Text style={styles.zoneRate}>
                        +{zone.h2Rate} H₂/turn
                      </Text>
                      <Text style={styles.zoneFlare}>
                        Flare:{" "}
                        {zone.flareRisk === "None" ? "—" : zone.flareRisk}
                      </Text>
                    </View>
                  </View>
                  <View style={styles.zoneDrones}>
                    {(deployedZones[zone.id] ?? []).map((did) => (
                      <Text key={did} style={styles.zoneDroneTag}>
                        {did}
                      </Text>
                    ))}
                  </View>
      ...
      {phase === "decision" && (
        <View style={[styles.decisionOverlay, panelStyle]}>
          <View style={styles.decisionPanel}>
            ...
            <TouchableOpacity
              style={[styles.choiceBtn, styles.choiceStation]}
              onPress={() => handleDecision("station")}
            >
              <Text style={styles.choiceLabel}>⬡ SETUP STATION</Text>
              <Text style={styles.choiceUpside}>
                + Passive H₂ +12/turn · System Influence +5/turn
              </Text>
              <Text style={styles.choiceUpside}>
                + Enables Research Station — tech tree unlocked
              </Text>
              <Text style={styles.choiceUpside}>
                + Rare Metal extraction becomes possible
              </Text>
              <Text style={styles.choiceDownside}>
                − Costs 400 H₂ to anchor · Takes 3 turns to build
              </Text>
              <Text style={styles.choiceDownside}>
                − No new drones until Station is online
              </Text>
            </TouchableOpacity>

            <TouchableOpacity
              style={[styles.choiceBtn, styles.choicePrinter]}
              onPress={() => handleDecision("printer")}
            >
              <Text style={styles.choiceLabel}>◈ ACTIVATE PRINTER</Text>
              <Text style={styles.choiceUpside}>
                + Print 2 drones immediately · +8 H₂/turn added
              </Text>
              <Text style={styles.choiceUpside}>
                + Faster expansion · More systems reachable sooner
              </Text>
              <Text style={styles.choiceDownside}>
                − Costs 600 H₂ upfront · No passive income
              </Text>
              <Text style={styles.choiceDownside}>
                − No tech research until a Station is built later
              </Text>
              <Text style={styles.choiceDownside}>
                − Rare Metals inaccessible until Era 2
              </Text>
            </TouchableOpacity>
          </View>
        </View>
      ...
      <Modal
        ...
        transparent
        animationType="slide"
        onRequestClose={() => setProbeOpen(false)}
      >
        <TouchableOpacity
          style={styles.modalBackdrop}
          activeOpacity={1}
          onPress={() => setProbeOpen(false)}
        />
        <View style={styles.modalSheet}>
          <View style={styles.modalHeader}>
            <View>
              <Text style={[styles.modalTitle, { color: "#60a0ff" }]}>
                PROMETHEUS
              </Text>
      ...
      {/* ── Star system modal ── */}
      <Modal
        visible={modalStar !== null}
        transparent
        animationType="slide"
        onRequestClose={() => setModalStar(null)}
      >
        <TouchableOpacity
          style={styles.modalBackdrop}
          activeOpacity={1}
          onPress={() => setModalStar(null)}
        />
        {modalStar && (
          <View style={styles.modalSheet}>
            <View style={styles.modalHeader}>
              <View>
                ...
                <Text style={styles.modalSub}>
                  {modalStar.starType} · {modalStar.distance} ly
                </Text>
              </View>
              <TouchableOpacity
                onPress={() => setModalStar(null)}
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              {[
                [
                  "H₂ DENSITY",
                  modalStar.hydrogenDensity,
                  modalStar.hydrogenDensity === "Very High"
                    ? "excellent"
                    : modalStar.hydrogenDensity === "High"
                      ? "positive"
                      : modalStar.hydrogenDensity === "Moderate"
                        ? "neutral"
                        : "dim",
                ],
                [
                  ...
                  modalStar.spectralClass === "D"
                    ? "excellent"
                    : modalStar.spectralClass === "C"
                      ? "positive"
                      : modalStar.spectralClass === "Unknown"
                        ? "warning"
                        : "neutral",
                ...
                [
                  "ASTEROID BELT",
                  modalStar.hasAsteroidBelt ? "Detected" : "None",
                  modalStar.hasAsteroidBelt ? "positive" : "dim",
                ],
                [
                  "ICE MOON",
                  modalStar.hasIceMoon ? "Present" : "None",
                  modalStar.hasIceMoon ? "positive" : "dim",
                ],
                [
                  "GAS GIANT",
                  modalStar.gasGiant ? "Present — scooping viable" : "None",
                ...
                <View key={label as string} style={styles.row}>
                  ...
                  <Text
                    style={[
                      styles.rowValue,
                      styles[clr as keyof typeof styles],
                    ]}
                  >
                    {value}
                  </Text>
              ...
              <View style={styles.row}>
                ...
                {modalStar.anomaly ? (
                  <Text style={[styles.rowValue, styles.warning]}>
                    ⚠ {modalStar.anomaly}
                  </Text>
                ) : (
                  <Text style={[styles.rowValue, styles.dim]}>
                    None detected
                  </Text>
                )}
              </View>
              <Text style={styles.sectionHead}>
                PLANETS ({modalStar.planetCount})
              </Text>
              {modalStar.planets.map((p, idx) => (
                <View
                  ...
                  style={[
                    styles.planetCard,
                    idx === modalStar.planets.length - 1 && styles.rowLast,
                  ]}
                >
                  ...
                  <View style={styles.planetRow}>
                    ...
                    {p.rareMetal && (
                      <Text style={[styles.planetDetail, styles.excellent]}>
                        ◆ Rare Metal trace
                      </Text>
                    )}
                  </View>
                ...
                <ScrollView showsVerticalScrollIndicator={false}>
                  {pd.rows.map((row, idx) => (
                    <View
                      ...
                      style={[
                        styles.row,
                        idx === pd.rows.length - 1 && styles.rowLast,
                      ]}
                    >
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
              ...
              return (
                ...
                >
                  <View style={styles.zoneModalLeft}>
                    <Text style={[styles.zoneModalName, { color: pd.color }]}>
                      {pd.label.toUpperCase()}
                    </Text>
                  </View>
                  <View style={styles.zoneModalRight}>
                    {orbitRow && (
                      <Text style={styles.zoneModalDesc}>
                        {orbitRow.label}: {orbitRow.value}
                      </Text>
                    )}
                    {massRow && (
                      <Text style={styles.zoneModalDesc}>
                        {massRow.label}: {massRow.value}
                      </Text>
                    )}
                    {habRow && (
                      <Text style={[styles.zoneModalDesc, { color: pd.color }]}>
                        {habRow.label}: {habRow.value}
                      </Text>
                    )}
                  </View>
                </View>
                ...
                >
                  <View style={styles.zoneModalLeft}>
                    <View style={styles.zoneLabelRow}>
                      <Text style={[styles.planetDot, { color: planetColor }]}>
                        ●
                      </Text>
                  ...
                  <View style={styles.zoneModalRight}>
                    <Text style={styles.zoneModalRate}>
                      +{zone.h2Rate} H₂/t
                    </Text>
                    {(deployedZones[zone.id] ?? []).map((did) => (
                      <Text key={did} style={styles.zoneModalDrone}>
                        {did}
                      </Text>
                    ))}
                  </View>
            ...
            <View style={[styles.row, styles.rowLast]}>
              <Text
                style={[
                  styles.rowValue,
                  { textAlign: "left", fontSize: 11, lineHeight: 18 },
                ]}
              >
                {
      ...
      <Modal
        visible={initModalOpen}
        transparent
        animationType="slide"
        onRequestClose={() => setInitModalOpen(false)}
      >
        <TouchableOpacity
          style={styles.modalBackdrop}
          activeOpacity={1}
          onPress={() => setInitModalOpen(false)}
        />
        <View
          style={[styles.modalSheet, { zIndex: 60, pointerEvents: "auto" }]}
        >
          <View style={styles.modalHeader}>
            <View style={styles.initHeaderLeft}>
              <Text style={styles.modalTitle}>INITIALIZE TORCH</Text>
              <Text style={styles.modalSub}>
                Select primary directive to begin
              </Text>
            </View>
            <View style={styles.initHeaderRight}>
              <View style={styles.initHudItem}>
                <Text style={styles.initHudLabel}>ENERGY</Text>
                <Text style={styles.initHudSub}>
                  H<Text style={styles.initHudSubscript}>2</Text>
                </Text>
                <Text style={styles.initHudValue}>
                  {h2Display.toLocaleString()}
                </Text>
              </View>
              <View style={styles.initHudItem}>
                <Text style={styles.initHudLabel}>RARE</Text>
                <Text style={styles.initHudSub}>METALS</Text>
                <Text
                  style={[
                    styles.initHudValue,
                    { color: rareMetals > 0 ? "#ffd580" : "#4a5a6a" },
                  ]}
                >
                  {rareMetals}
                </Text>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            <TouchableOpacity
              style={[styles.choiceBtn, styles.choiceDrone]}
              onPress={() => {
                setDirective("printer");
                setH2((prev) => prev - 200);
            ...
            >
              ...
              <Text style={styles.choiceUpside}>
                + Printer enabled for later production · No Station
              </Text>
              <Text style={styles.choiceDownside}>
                − Costs 200 H₂ upfront · Takes 1 turn
              </Text>
              <Text style={styles.choiceDownside}>
                − No passive income · Rare Metals until Era 2
              </Text>
            </TouchableOpacity>
            ...
            >
              ...
              <Text style={styles.choiceUpside}>
                + Enables parallel drone production · Build queue open
              </Text>
              <Text style={styles.choiceUpside}>
                + Faster mid-game expansion capability
              </Text>
              <Text style={styles.choiceDownside}>
                − Costs 350 H₂ upfront · No drones or passive income
              </Text>
              <Text style={styles.choiceDownside}>
                − Takes 2 turns · No tech research
              </Text>
            </TouchableOpacity>
            ...
            >
              ...
              <Text style={styles.choiceUpside}>
                + Rare Metal extraction possible
              </Text>
              <Text style={styles.choiceDownside}>
                − Costs 500 H₂ to anchor · Takes 3 turns to build
              </Text>
              <Text style={styles.choiceDownside}>
                − No new drones until Station is online
              </Text>
            ...
            <TouchableOpacity
              style={[styles.choiceBtn, styles.choiceProbe]}
              onPress={() => {
                setDirective("printer");
                setH2((prev) => prev - 1800);
                setInitModalOpen(false);
                startInitSequence();
              }}
            >
              ...
              <Text style={styles.choiceUpside}>
                + 4 Drones + Printer · Full expansion package
              </Text>
              <Text style={styles.choiceUpside}>
                + Ready for immediate system exploration
              </Text>
              <Text style={styles.choiceDownside}>
                − Costs 1,800 H₂ · Takes 3 turns to assemble
              </Text>
              <Text style={styles.choiceDownside}>
                − No Station · Must build research later
              </Text>
            </TouchableOpacity>
            ...
            >
              ...
              <Text style={styles.choiceUpside}>
                + 4 Drones + Printer + Station · Maximum starting power
              </Text>
              <Text style={styles.choiceUpside}>
                + Station comes online with 2 Drones · Full research enabled
              </Text>
              <Text style={styles.choiceUpside}>
                + Immediately expand to new star systems
              </Text>
              <Text style={styles.choiceDownside}>
                − Costs 2,200 H₂ · The "go wide" nuclear option
              </Text>
              <Text style={styles.choiceDownside}>
                − Requires rebuilding local economy after
              </Text>
            </TouchableOpacity>
          </ScrollView>
        </View>
          ...
          <View style={styles.aiModalSheet}>
            ...
            >
              {aiMessages.length === 0 && (
                <Text style={styles.aiEmptyText}>
                  TORCH AI online. I have full awareness of the Proxima Centauri
                  system, your current resources, and all deployed units. How
                  can I assist, Commander?
                </Text>
              )}
              {aiMessages.map((msg, idx) => (
                ...
                >
                  <Text style={styles.aiMessageRole}>
                    {msg.role === "user" ? "COMMANDER" : "TORCH AI"}
                  </Text>
                  <Text style={styles.aiMessageText}>{msg.content}</Text>
                </View>
              ))}
              {aiLoading && (
                <View style={[styles.aiMessage, styles.aiMessageAssistant]}>
                  <Text style={styles.aiMessageRole}>TORCH AI</Text>
                  <Text style={[styles.aiMessageText, { opacity: 0.5 }]}>
                    Processing...
                  </Text>
                </View>
              )}
            ...
            <View style={styles.aiInputRow}>
              ...
              <TouchableOpacity
                style={[
                  styles.aiSendBtn,
                  (aiLoading || !aiInput.trim()) && styles.aiSendBtnDisabled,
                ]}
...
const styles = StyleSheet.create({
  ...
  choiceProbe: {
    borderColor: "#8a4a8a",
    backgroundColor: "rgba(40,20,40,0.6)",
  },
  choiceLabel: {
    color: "#a8d8ff",
    fontSize: 16,
